...
//	private SignatureComparator signatureComparator = new SimilarSignatures();
	...
	public SatelliteImage(int minX, int minY, int width, int height,
			...
			SampleModel tileSampleModel, ColorModel colorModel, List<String> bandsPaths) {
		...
		properties.put("GAINS/BIASES", "1.26880/-0.0100  2.98126/-0.0232  1.76186/-0.0078  2.81771/-0.0193  0.65277/-0.0080  3.20107/0.25994  0.44375/-0.0040");
		
		this.bands = new ArrayList<Integer>();
		int con =0;
		for (String string : bandsPaths) {
			if (con < 3)
				this.bands.add(Integer.valueOf(string.substring(string.length()-5, string.length()-4)));
			con++;
		}
...
//		para que se rompa BIEN ROTO!
	...
	private byte[] getNDs(int fromX, int toX, int fromY, int toY, int band) {
		...
		for (int y=fromY;y<toY;++y)
			for (int x=fromX;x<toX;++x)
				...
				ret[i++] = (byte)db.getElem(band + x * cantBandas + getWidth() * y *cantBandas);
	...
	private double []getBiases() {
		...
		//extraigo los biases, que están a la derecha de la "/"
		String []gbArr = gainsBiases.split("  ");
		int con = 0;
		
		for (int i=0; i<gbArr.length; ++i)
			if (isBandPresent(i))
				ret[con++] = Double.valueOf(gbArr[i].split("/")[1]).doubleValue();
	...
	/*private String getBaseDir() {
		...
		private int getMaxBandsAllowed() {
			return maxBandsAllowed;
		}
	*/
		private double []getGains() {
			...
			for (int i=0; i<gbArr.length; ++i)
				if (isBandPresent(i))
					ret[con++] = Double.valueOf(gbArr[i].split("/")[0]).doubleValue();
	 ...
	 * @param bandsList 
	 * @param numBand El número de banda, de 1 a 7 en <i>Landsat 5</i>, de 1 a 8 en <i>Landsat 7</i>
	 * @param pos Posición en la que se quiere poner esta banda, para jugar con el <i>falso color</i>
	 * @throws TooManyBandsException Si se intenta agregar una banda cuando ya se llegó al límite para
	 * esta imagen 
	 */
	/*public void addBand(int numBand, int pos) throws TooManyBandsException{
		int cantBandas = getCantBands();
		//primero hay que fijarse que la imagen no esté llena
		if (cantBandas == getMaxBandsAllowed())
			throw new TooManyBandsException();
		
		String bandString = getBaseDir()+"/BAND"+numBand+".DAT";
		//ahora, 1 de cada X bytes va a ser el byte nuevo que voy a sacar de la banda que agrego
		bandsFiles.add(pos, bandString);
		
		int width = Integer.valueOf(properties.get("PIXELS PER LINE"));
		int height = Integer.valueOf(properties.get("LINES PER IMAGE"));
		
		PlanarImage pi = fcr.read(bandsFiles, width, height);
		
		
		Raster r = pi.getAsBufferedImage().getRaster();
		super.setData(r);
	...
	*/
		...
		for (int y=fromY;y<toY;++y)
			for (int x=fromX;x<toX;++x)
				for (int b=0; b<cantBandas; ++b){
					ret[i++] = (byte)db.getElem(b + x * cantBandas + getWidth() * y * cantBandas);
				}
	...
	/**
	 * Crea una copia de esta imagen y le aplica la corrección en base a l <b>Reflectancia</b>.
	 */
	 ...
	 * Crea una copia de esta imagen y le aplica la corrección en base a l <b>Reflectancia</b>.
	 */
	public List<SatelliteImage> getReflectanceCorrected(int fromX,int toX,int fromY, int toY){
...
//				dbCorrected.setElem(i, (byte) (rhos[i]*255.0));
		...
		/*
		double[] radiances = getRadiance(fromX, toX, fromY, toY);
		int cantBands = getCantBands();
		int bufSize = db.getSize()/cantBands;

		double []rhos = rayleigh.getReflectance(bands, radiances);
		for (int band=0; band<cantBands; ++band){
			DataBufferByte dbCorrected = new DataBufferByte(bufSize);
			for (int i = band; i < rhos.length; i+=cantBands) {
				dbCorrected.setElem(i/cantBands, (byte) (rhos[i]*255.0));
			}
	...
	/**
	 * Dada una firma y un comparador, retorna 4 imagenes que corresponden a cada una de las 3 bandas de esta imagen 
	 * y a todas ellas unidas en una sola. Listas para poner en solapas ;)
	 * 
	 * @param comparator comparador de las firmas. Ya tiene adentro la firma original, por eso es que esta no se recibe como par�metro
	 * @return una lista con 4 imagenes: 3 de una sola banda y una de 3 bandas
	 */
	public List<SatelliteImage> getImageWithThisSignature(SignatureComparator comparator){
		List<SatelliteImage> ret = new ArrayList<SatelliteImage>(4);
		
		int[] originalData = getData().getDataBuffer().getOffsets();
		
		byte []correctedData = getByteRadiance(0, width, 0, height);//correctByRadiance(originalData);
	 ...
	 * @return
	 */
	private SatelliteImage makeCopy(byte[] newData, int fromX, int toX, int fromY,
			int toY) {
		int cantBands = getCantBands();
	...
	private SatelliteImage makeCopy(DataBufferByte dbCorrected, int fromX,
		...
		Raster raster = RasterFactory.createWritableRaster(sampleModel,dbCorrected,
				new Point(0,0));
	...
	private SatelliteImage makeCopy(byte[] newData, int fromX, int toX,
		...
		return makeCopy(new DataBufferByte(oneBandData,size),fromX,toX,fromY,toY,getBands()[indexBand]);
	}
	...
	/**
	 * Crea una copia de esta imagen y le aplica la corrección en base a l <b>Reflectancia</b>.
	 * Los parámetros de entrada sirven para especificar una porción de la imagen. NO
	 * están en Lat-Lon, sino en <b>pixeles</b>.
	 */
	public List<SatelliteImage> getRadianceCorrected(int fromX,int toX,int fromY, int toY){
		List<SatelliteImage> ret = new ArrayList<SatelliteImage>();
...
//			ret.add(makeCopy2(db,fromX,toX,fromY,toY,band));
			ret.add(makeCopy2(dbCorrected,fromX,toX,fromY,toY,i));
		}
		return ret;
	}

	
	private SatelliteImage makeCopy(DataBufferByte dbCorrected, int fromX, int toX, int fromY, int toY,int band) {
		// Create an pixel interleaved data sample model.
		int cantBands = dbCorrected.getSize() / (toX-fromX) / (toY-fromY);
		SampleModel sampleModel =
			RasterFactory.
	...
	public SatelliteImage getOneBand(int band){
		...
		return makeCopy2(getOneBandData(band),0, getWidth(),0, getHeight(),band);
	}

	private SatelliteImage makeCopy2(DataBufferByte dbCorrected, int fromX, int toX,
			int fromY, int toY,int band) {
		int width = toX - fromX;
		...
		// Create a TiledImage using the SampleModel.
		SatelliteImage ret = new SatelliteImage(0,0,width,height,0,0,
				sampleModel,colorModel,new ArrayList<String>());
		// Set the data of the tiled image to be the raster.
		ret.setData(raster);
		return ret;
	}
